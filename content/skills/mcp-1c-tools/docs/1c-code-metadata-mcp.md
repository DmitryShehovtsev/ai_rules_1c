...
| **get_metadata_details** | `object_name`, `detail_level`, `max_chars`, `max_items`, `cursor` | Paged structure: attributes with types, tabular parts, synonyms, properties | Known canonical `full_path`, e.g. `'Документы.НачислениеЗарплаты'`; choose the smallest sufficient detail level |
...
2. Start `get_metadata_details` with explicit character **and** item bounds, for example `max_chars=12000, max_items=40`. Use `detail_level="outline"` for names/types; use `"full"` when synonyms/properties are requested. Tune bounds to the connected client's response limit; these values are a starting point, not a guarantee against file spill. Do not fetch an entire large object just to identify it.
...
4. The current payload has `data.attributes` as a list and `data.tabular_parts` as a **map from tabular-part name to a list of column records**, not to an object with an `attributes` property. Column records contain `name`, `type`, and, in the full projection, `synonym`. Read structured output directly when the client exposes it. If the client only supplies a dumped-file path, inspect its actual wrapper once and extract the required fields in one bounded pass; do not repeatedly guess the shape or re-read the whole JSON. Use the platform's safe quoting and UTF-8 handling for Cyrillic.
...
6. Use text views, section filters or field projections only if the connected schema exposes them. The current `get_metadata_details` contract has no `format="text"` or tabular-part selector; instructions cannot enable an unimplemented server feature. MCP's JSON transport envelope does not require the text payload itself to be serialized JSON.
